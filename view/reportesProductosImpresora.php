...
<body>
    ...
    $sql = $conectar->prepare("SELECT codigo_productos,nombre_productos,cantidad_productos,pre.nombre_presentacion, DATE_FORMAT(p.fecha_crea, '%d/%m/%y') as Fecha FROM productos p INNER JOIN presentacion pre ON p.presentacion_productos = pre.id_presentacion WHERE esActivo = 1 and tipo_productos = 5  ORDER BY  codigo_productos;");
    $sql->execute();
